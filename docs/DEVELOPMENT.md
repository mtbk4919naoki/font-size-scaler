...
列: レベル / 用途 / SP / PC / {previewVw}px / ジャンプ率 / 備考 / CSS

- ジャンプ率はプレビュー vw 列の値同士の比率（1 列）
...
目標値との差は `formatTargetDiff()` — 絶対 px · 相対 %。相対差 ±5% 以上で `.cell-diff-warn`（赤字）。

関連関数: `getPracticalChecks()`, `closestIntegerLevel()`, `renderPracticalRow()`, `formatTargetDiff()`
...
| 実用チェック | `getPracticalChecks()`, `formatTargetDiff()`, `renderPractical()` |
