...
window.FSSPreviewSample = (function () {
  ...
  function create(deps) {
    ...
    function pst(level, c, vw, tag, className, content) {
      const lv = resolvePreviewLevel(level);
      return `<${tag} class="preview-scale-text ${className}" data-preview-level="${lv}" style="${previewTextStyle(lv, c, vw)}">${escapeHtml(content)}</${tag}>`;
    }
    ...
    function featureCard(c, vw, titleKey, bodyKey, metaKey, imgClass) {
      return `<article class="ps-feature-card">
        ...
        <div class="ps-feature-card-body">
          ${pst(1, c, vw, 'h4', 'ps-feature-card-title', t(titleKey))}
          ${pst(0, c, vw, 'p', 'ps-feature-card-text', t(bodyKey))}
          ${pst(-1, c, vw, 'p', 'ps-feature-card-meta', t(metaKey))}
        </div>
      </article>`;
    }
      ...
      return `<div class="preview-sample"${wrapAttr}>
        <div class="ps-shell">
          ...
          <aside class="ps-sidebar" id="psSidebar" aria-label="Sidebar">
            ${pst(-1, c, vw, 'p', 'ps-sidebar-label', t('sampleSidebarDoc'))}
            <ul class="ps-sidebar-list">
              ${sidebarItem(-0.5, t('sampleSidebarNav1'), true)}
              ${sidebarItem(-0.5, t('sampleSidebarNav2'))}
              ${sidebarItem(-0.5, t('sampleSidebarNav3'))}
            </ul>
            ${pst(-1, c, vw, 'p', 'ps-sidebar-label', t('sampleSidebarProject'))}
            <ul class="ps-sidebar-list">
              ${sidebarItem(-0.5, t('sampleSidebarNav2'))}
              <li class="ps-sidebar-nested">
                <ul class="ps-sidebar-sublist">
                  ${sidebarItem(-1, t('sampleSidebarSub1'))}
                  ${sidebarItem(-1, t('sampleSidebarSub2'))}
                </ul>
              </li>
            </ul>
          </aside>
          ...
          <div class="ps-main">
            ...
            ${pst(-2, c, vw, 'p', 'ps-breadcrumb', t('sampleBreadcrumb'))}
            ${pst(4, c, vw, 'h1', 'ps-page-title', t('samplePageTitle'))}
            ${pst(0.5, c, vw, 'p', 'ps-page-lead', t('samplePageLead'))}

            <section class="ps-kv">
              <div class="ps-kv-media" aria-hidden="true"></div>
              <div class="ps-kv-copy">
                ${pst(-1, c, vw, 'p', 'ps-kv-eyebrow', t('sampleKvEyebrow'))}
                ${pst(5, c, vw, 'h2', 'ps-kv-title', t('sampleKvTitle'))}
                ${pst(0.5, c, vw, 'p', 'ps-kv-lead', t('sampleKvLead'))}
              </div>
            </section>

            <section class="ps-section">
              ${pst(3, c, vw, 'h2', 'ps-section-title', t('sampleSectionOverview'))}
              <div class="ps-media">
                <div class="ps-media-image" aria-hidden="true"></div>
                <div class="ps-media-body">
                  ${pst(2, c, vw, 'h3', 'ps-media-title', t('sampleMediaTitle'))}
                  ${pst(0, c, vw, 'p', 'ps-media-text', t('sampleMediaBody'))}
                  ${pst(-1, c, vw, 'p', 'ps-media-caption', t('sampleMediaCaption'))}
                </div>
              </div>
            </section>

            <section class="ps-section">
              ${pst(3, c, vw, 'h2', 'ps-section-title', t('sampleSectionArticles'))}
              <ul class="ps-article-list">
                <li class="ps-article-item">
                  ${pst(1, c, vw, 'h4', 'ps-article-title', t('sampleArticle1Title'))}
                  ${pst(0, c, vw, 'p', 'ps-article-body', t('sampleArticle1Body'))}
                  ${pst(-1, c, vw, 'p', 'ps-article-meta', t('sampleArticle1Meta'))}
                </li>
                <li class="ps-article-item">
                  ${pst(1, c, vw, 'h4', 'ps-article-title', t('sampleArticle2Title'))}
                  ${pst(0, c, vw, 'p', 'ps-article-body', t('sampleArticle2Body'))}
                  ${pst(-1, c, vw, 'p', 'ps-article-meta', t('sampleArticle2Meta'))}
                </li>
              </ul>
            </section>

            <section class="ps-section">
              ${pst(3, c, vw, 'h2', 'ps-section-title', t('sampleSectionFeatured'))}
            ...
            <section class="ps-section">
              ${pst(3, c, vw, 'h2', 'ps-section-title', t('sampleSectionCards'))}
              <div class="ps-cards">
                <article class="ps-card">
                  ${pst(1, c, vw, 'h4', 'ps-card-title', t('sampleCard1Title'))}
                  ${pst(0, c, vw, 'p', 'ps-card-body', t('sampleCard1Body'))}
                  ${pst(-2, c, vw, 'p', 'ps-card-meta', t('sampleCard1Meta'))}
                </article>
                <article class="ps-card">
                  ${pst(1, c, vw, 'h4', 'ps-card-title', t('sampleCard2Title'))}
                  ${pst(0, c, vw, 'p', 'ps-card-body', t('sampleCard2Body'))}
                  ${pst(-2, c, vw, 'p', 'ps-card-meta', t('sampleCard2Meta'))}
                </article>
              </div>
              <div class="ps-actions">
                ${pst(-0.5, c, vw, 'span', 'ps-btn ps-btn-primary', t('sampleBtnPrimary'))}
                ${pst(-0.5, c, vw, 'span', 'ps-btn ps-btn-secondary', t('sampleBtnSecondary'))}
              </div>
            </section>
          </div>
        </div>
        ...
        <footer class="ps-footer">
          <div class="ps-footer-cols">
            <div>
              ${pst(-1, c, vw, 'p', 'ps-footer-heading', t('sampleFooterCol1'))}
              <ul class="ps-footer-list">${footerLinks(-1)}</ul>
            </div>
            <div>
              ${pst(-1, c, vw, 'p', 'ps-footer-heading', t('sampleFooterCol2'))}
              <ul class="ps-footer-list">${footerLinks(-1)}</ul>
            </div>
            <div>
              ${pst(-1, c, vw, 'p', 'ps-footer-heading', t('sampleFooterCol3'))}
              <ul class="ps-footer-list">${footerLinks(-1)}</ul>
            </div>
          </div>
          ${pst(-2, c, vw, 'p', 'ps-footer-copy', t('sampleFooterCopy'))}
        </footer>
      </div>`;
    }
